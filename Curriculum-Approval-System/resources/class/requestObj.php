<?php
require_once(CLASS_PATH."emailObj.php");
...
class Request
{
        ...
        $sql = "SELECT groupMember.userId 
                    FROM `groupMember`, `request`, `group` 
                        WHERE group.groupId = '$groupId' 
                            ...
                            AND groupMember.userId != request.userId";
    ...
    private function updateApprovalTable($userId, $submission)
    {
        ...
        $con = mysqli_connect(HOST, USERNAME, PASSWORD, DB_NAME) or die('Failed to connect to database');
        $submission = mysqli_real_escape_string($con, $submission);
        $sql  = "UPDATE `approval` 
    ...
    public function setCurrentApprover($value) { $this->currentApprover = $value; }
    public function getCurrentApprover() { return $this->currentApprover; }
